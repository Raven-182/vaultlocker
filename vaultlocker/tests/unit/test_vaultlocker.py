...
class TestVaultlocker(base.TestCase):
    ...
    @mock.patch.object(shell, '_verify_cluster_identity')
    @mock.patch.object(shell, '_vault_client')
    def test_identity_is_checked_before_login(
            self, _vault_client, _verify_cluster_identity, _login_to_vault):
        calls = []
        ...
        self.assertEqual(['identity', 'login', 'operation'], calls)
    ...
    @mock.patch.object(shell, 'get_config')
    @mock.patch.object(shell, 'encrypt')
    def test_main_retryable_failure_exit_code(
            self, _encrypt, _get_config, _exit):
        """A controlled, explicitly-handled failure is safe to retry."""
        _get_config.return_value = self.config
        ...
        _exit.assert_called_once_with(ExitCode.RETRYABLE_FAILURE)
    ...
    @mock.patch.object(shell, 'get_config')
    @mock.patch.object(shell, 'enroll')
    def test_main_retryable_failure_exit_code_for_validation(
    ...
    @mock.patch.object(shell, 'get_config')
    @mock.patch.object(shell, 'encrypt')
    def test_main_configuration_failure_is_retryable(
            self, _encrypt, _get_config, _exit):
    ...
    def test_failure_json_is_the_final_nonempty_stderr_line(
        ...
        self.assertEqual(ExitCode.RETRYABLE_FAILURE, error.exception.code)
        stderr_lines = [
    ...
    def test_main_unexpected_failure_exit_code(
            self, _encrypt, _get_config, _exit):
        """An exception vaultlocker did not recognize is not retryable."""
        _get_config.return_value = self.config
    ...
    def test_main_usage_error_is_not_the_retryable_exit_code(self):
        ...
        That must stay distinct from ``ExitCode.RETRYABLE_FAILURE``.
        """
        with self.assertRaises(SystemExit) as error:
            shell.main()
        ...
        self.assertNotEqual(
            ExitCode.RETRYABLE_FAILURE, error.exception.code,
        )
...
class TestReadLuksUuid(base.TestCase):
    ...
    def test_returns_uuid_for_existing_header(self, _luks_uuid):
        ...
        self.assertEqual(
            'existing-uuid', shell._read_luks_uuid('/dev/sdb'),
        )
    ...
    def test_returns_none_for_the_no_header_exit_code(self, _luks_uuid):
        ...
        self.assertIsNone(shell._read_luks_uuid('/dev/sdb'))
...
class TestCanUnlockDevice(base.TestCase):
    ...
    def test_true_when_the_key_works(self, _luks_test_key):
        self.assertTrue(shell._can_unlock_device('key', '/dev/sdb'))
    ...
    def test_false_when_the_key_does_not_work(self, _luks_test_key):
        self.assertFalse(shell._can_unlock_device('key', '/dev/sdb'))

    @mock.patch.object(shell.dmcrypt, 'luks_test_key')
    def test_false_when_the_probe_itself_fails(self, _luks_test_key):
    ...
    @mock.patch.object(shell.dmcrypt, 'luks_test_key')
    def test_false_when_the_probe_times_out(self, _luks_test_key):
...
class TestReuseEncryptedDevice(base.TestCase):
    ...
    @mock.patch.object(shell, '_can_unlock_device', return_value=True)
    @mock.patch.object(shell, '_read_vault_key')
    @mock.patch.object(shell, '_vault_store')
    def test_reuses_when_the_managed_key_works(
            self, _vault_store, _read_vault_key, _can_unlock_device,
            _open_and_register):
        ...
        result = shell._reuse_encrypted_device(
            args, mock.MagicMock(), mock.MagicMock(), '/dev/sdb',
            'existing-uuid',
        )

        _can_unlock_device.assert_called_once_with('managed-key', '/dev/sdb')
    ...
    @mock.patch.object(shell, '_can_unlock_device', return_value=False)
    @mock.patch.object(shell, '_read_vault_key')
    @mock.patch.object(shell, '_vault_store')
...
class TestFormatCompleted(base.TestCase):
    ...
    def test_false_when_the_key_does_not_unlock_it(
            self, _luks_uuid, _can_unlock_device):
        self.assertFalse(
            shell._format_completed('/dev/sdb', 'the-uuid', 'the-key'),
        )
...
class TestEncryptBlockDevice(base.TestCase):
    ...
    @mock.patch.object(shell, '_reuse_encrypted_device')
    @mock.patch.object(shell, '_read_luks_uuid', return_value=None)
    ...
    @mock.patch.object(shell, '_format_device')
    @mock.patch.object(shell, '_reuse_encrypted_device')
    @mock.patch.object(shell, '_read_luks_uuid',
...
class TestEnrollBlockDevice(base.TestCase):
    ...
    def test_succeeds_without_the_original_credential_when_already_enrolled(
        ...
        get_current_key = mock.MagicMock(
        ...
        with mock.patch.object(
                shell, '_can_unlock_device', return_value=True,
        ) as key_check:
            result = shell._enroll_block_device(
                args, mock.MagicMock(), mock.MagicMock(), get_current_key,
            )

        get_current_key.assert_not_called()
    ...
    def test_generates_and_adds_a_managed_key_when_none_exists(
        ...
        _open_and_register.return_value = {'luks_uuid': 'test-uuid'}
        get_current_key = mock.MagicMock(return_value=b'existing-key')
        ...
        get_current_key.assert_called_once()
    ...
    def test_rejects_an_existing_credential_that_does_not_unlock_it(
            self, _luks_uuid, _vault_store, _read_vault_key):
        get_current_key = mock.MagicMock(return_value=b'wrong-key')

        args = mock.MagicMock()
        args.block_device = ['/dev/sdb']

        with mock.patch.object(
                shell, '_can_unlock_device', return_value=False,
        ):
            with self.assertRaises(
...
class TestEnrollHandler(base.TestCase):
    ...
    def test_credential_is_cached_across_repeated_use(
        ...
        operation = _do.call_args[0][0]
        get_current_key = operation.keywords['get_current_key']

        # Repeated reads use the cached key.
        first = get_current_key()
        second = get_current_key()
...
class TestHandlersAndConfig(base.TestCase):
    ...
    def test_enroll_handler_delegates(self, _do):
        ...
        self.assertTrue(callable(operation.keywords['get_current_key']))
        self.assertIs(args, _do.call_args[0][1])
        self.assertIs(config, _do.call_args[0][2])
        self.assertEqual({'luks_uuid': 'test-uuid'}, result)
...
class TestClusterIdentity(base.TestCase):
    ...
    @mock.patch.object(shell, '_login_to_vault')
    @mock.patch.object(shell, '_vault_client')
    def test_verification_failure_runs_no_login_or_operation(
        ...
        _login_to_vault.assert_not_called()
        operation.assert_not_called()
